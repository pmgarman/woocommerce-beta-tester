...
class WC_GitHub_Updater {
	...
	public function set_defaults() {
		...
		if ( ! isset( $this->config['new_version'] ) )
			$this->config['new_version'] = $this->get_latest_commit();
	...
	public function get_new_version() {
		...
		if ( !isset( $version ) || !$version || '' == $version ) {
			...
			// refresh every hour
			set_site_transient( $this->config['slug'].'_new_version', $version, 60*60*1 );
		}
	...
	public function get_github_data() {
		...
		if ( ! isset( $github_data ) || ! $github_data || '' == $github_data ) {
			...
			// refresh every hour
			set_site_transient( $this->config['slug'].'_github_data', $github_data, 60*60*1);
		}
	...
	/**
	 * Get GitHub Commits from the specified repository
	 *
	 * @since 1.0
	 * @return array $github_data the data
	 */
	public function get_latest_commit() {
		$last_commit = get_site_transient( $this->config['slug'].'_last_commit' );
	...
	public function api_check( $transient ) {
		...
		if ( get_option( 'woocommerce_commit_installed' ) != $this->get_latest_commit() ) {
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) {
		...
		update_option( 'woocommerce_commit_installed' );
